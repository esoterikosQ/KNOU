...
```{r 4-1-2}
# 다중선형회귀분석
model_lm <- lm(Salary~., dt)
res_lm <- sum(((model_lm$residuals)^2))/nrow(dt) # MSE = 92017.87
summary(model_lm)
```

```{r 4-1-3}
# ridge
## preprocessing variables to matrix 
...
## plotting
plot(cv_ridge, main = 'Ridge regression cross validation error(10 fold)')
## best MSE
best_ridge <- cv_ridge$lambda.min
best_ridge # MSE = 238.0769
```
...
library(HSAUR)
data(watervoles)
write.csv(watervoles, file = 'ML_3.csv', row.names = F)
```
